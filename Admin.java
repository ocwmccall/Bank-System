...
public class Admin extends banksys{
    public Admin(banksys input) {
        set_ID(input.get_ID());
    ...
    public void home() throws SQLException {
        System.out.println("------------------------------\n" +
                "Hello Administrator " + Objects.toString(get_ID()));
        while(get_logged()) {
            System.out.println("------------------------------\n" +
                    "(1) Create Employee Account \n----------\n" +
                    "(2) Reset Employee PIN\n----------\n" +
                    "(3) Reset PIN\n----------\n" +
                    "(4) Logout \n----------\n" +
    ...
    private void create_employee() {
        System.out.println("----------\nPlease ask Employee for a desired userID or enter -1 to go back.\n----------");
        String newID;
        Boolean flag = false;
        while (true) {
            while (true) {
                ...
                try (PreparedStatement prep = get_conn().prepareStatement(sql)) {
                    ...
                    if (0 != results.getInt("userID")) {
                        System.out.println("This userID is already taken, please ask the Employee for another.");
                        continue;
                    }
            ...
            try (Connection conn = get_conn()) {
                ...
                System.out.println("----------\nEmployee " + newID + " has been created with PIN set to 0000");
                break;
            }catch (SQLException e) {
                e.printStackTrace();
            }catch (ArithmeticException a) {
                System.out.println("Entry wasnt added to table");
                a.printStackTrace();
            }
        }
    }

    private void reset_employeePIN() {
        System.out.println("----------\nPlease enter ID of employee to reset their PIN or -1 to go back\n----------");
        String target;
        while (true) {
            target = four_digits();
            if (target.equals("-1")) {
            ...
            try (Statement stat = get_conn().createStatement()) {
                ...
                if (type != 2) {
                    System.out.println("Cannot reset non-employee PIN with this function," +
            ...
            try (PreparedStatement prep = get_conn().prepareStatement(sql)) {
                prep.executeUpdate();
                System.out.println("----------\nEmployee " + target + " has had their PIN set to 0000.");
                break;
            } catch (SQLException e) {
                e.printStackTrace();
            }
